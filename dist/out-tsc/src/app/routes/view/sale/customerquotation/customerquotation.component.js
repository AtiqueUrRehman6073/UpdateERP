...
let CustomerquotationComponent = class CustomerquotationComponent {
    constructor(activatedroute, messageService, fb, confirmation, translate, router, masterapi, accountapi) {
        ...
        this.salesManList = [];
        this.salesManArry = [];
        this.customerQuotationarry = [];
        this.hotRegisterer = new HotTableRegisterer();
        this.hotid = 'customerQuotationEntry';
    ...
    getLocation(id) {
        var _a;
        return (_a = this.locationList) === null || _a === void 0 ? void 0 : _a.find(k => k.value === id).label;
    }
    getSalesMan(id) {
        var _a;
        return (_a = this.salesManList) === null || _a === void 0 ? void 0 : _a.find(k => k.value === id).label;
    }
    get f() {
    ...
    initializeControls() {
        ...
        this.customerQuotationentry.afterRemoveRow = (index, amount) => {
    ...
    savecustomerQuotation() {
        ...
        const savedData = this.customerQuotationentry.data.filter(k => k.hasOwnProperty('item'));
        const customerQuote = this.customerQuotationFormGroup.value;
    ...
    updateFormGrid(CustQuote) {
        ...
        const gridData = CustQuote.CustomerQuotationDetails.map(k => {
            const accountId = k.customerQuotationDetailsItemId != null ? this.itemArry.find(l => l.match(k.customerQuotationDetailsItemId)) : null;
            return {
    ...
    GetSavedCustomerQuotationDetails(id) {
        this.index = 0;
    ...
    get IsValidGrid() {
        if (this.customerQuotationentry.data.length > 0) {
            ...
            return Object.values(this.customerQuotationentry.data).some((k) => k.item != null && k.gamount != null);
            // });
        }
        return false;
    }
};
CustomerquotationComponent = __decorate([
    Component({
...
export { CustomerquotationComponent };
export const vchrnoAsyncValidator = (acctService, time = 800) => {
    return (input) => {
        if (input.value != null && input.value.trim() !== '') {
            return timer(time).pipe(switchMap(() => acctService.CheckVnoExist(input.value)), map(res => {
                return res.result === false ? null : { Exist: true };
            }));
        }
    };
};
//# sourceMappingURL=customerquotation.component.js.map