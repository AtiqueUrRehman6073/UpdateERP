...
let PaymentvoucherComponent = class PaymentvoucherComponent {
    constructor(activatedroute, messageService, fb, confirmation, translate, router, masterapi, accountapi) {
        ...
        this.paymentVoucherarry = [];
        this.hotRegisterer = new HotTableRegisterer();
        this.hotid = 'paymtVouchrEntry';
    ...
    initializeControls() {
        ...
        this.paymentvoucherentry.afterRemoveRow = (index, amount) => {
    ...
    savePaymentVoucher() {
        ...
        const savedData = this.paymentvoucherentry.data.filter(k => k.hasOwnProperty('account'));
        const paymtVoucher = this.paymentVoucherFormGroup.value;
        ...
        if (this.btnFlag.save) {
            ...
            paymtVoucher.accountsTransactions.filter(x => x.accountsTransactionsDebit > 0)
                ...
                const acctTransEntry = this.savedPaymentVoucher.accountsTransactions
    ...
    updateFormGrid(pymt) {
        ...
        const gridData = pymt.paymentVoucherDetails.map(k => {
            const accountId = k.paymentVoucherDetailsAcNo != null ? this.acctArry.find(l => l.match(k.paymentVoucherDetailsAcNo)) : null;
            const jobId = (k.paymentVoucherDetailsJobId != null) ? this.jobArry.find(l => l.match(k.paymentVoucherDetailsJobId.toString())) : null;
            return {
                account: accountId,
    ...
    get IsValidGrid() {
        if (this.paymentvoucherentry.data.length > 0) {
            ...
            return Object.values(this.paymentvoucherentry.data).some((k) => k.account != null && k.debit != null);
            // });
        }
        return false;
    }
...
export { PaymentvoucherComponent };
...
//# sourceMappingURL=paymentvoucher.component.js.map